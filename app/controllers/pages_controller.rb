class PagesController < ApplicationController
  ...
  def index
    ...
    @projects = Project.joins(:images_blobs).where(id:project_ids).where(active_storage_blobs: { content_type: ["image/png", "image/jpeg"] }).where("expiration >= ?", DateTime.now).where(status:"Visible").order(created_at: :desc).limit(9)
  end
end
